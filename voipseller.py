...
#
    ...
    [InlineKeyboardButton(text = '⚙️ Pannello Admin ⚙️', callback_data = 'pannellodiamministrazione')],
        ...
        await app.send_message(chat_id = message.chat.id, text =  f'''Ciao, benvenuto nel tuo bot''', reply_markup = start_btn)  
...
def homesdfas(_, query):
    ...
    query.message.edit(f'**Ciao, benvenuto nel tuo bot**', reply_markup = start_btn)
...
def homecompra(_, query):
    ...
    query.message.edit(f'🔎 Seleziona il servizio per il quale utilizzerai il VoIP', reply_markup = back_to_start_btn)
...
def pannellodiamministrazione(_, query):
    ...
    [InlineKeyboardButton(text = f'🔝 Ricarica', url = 'https://sms-activate.ru/es/buy')],
    [InlineKeyboardButton(text = f'👮‍♂️ Admin', callback_data = 'admin')],
    [InlineKeyboardButton(text = '🏘 Menù Principale 🏘', callback_data = 'home')],
    ])
    query.message.edit(f'**⚙️ Pannello Admin ⚙️**', reply_markup = back_to_start_btn)

@app.on_callback_query(callback_data_filter("admin"))
def admin(_, query):
    query.answer('Prossimamente...')
...
def telegramvoip(_, query):
    ...
    query.message.edit(f'🔎 Seleziona la nazione del VoIP per Telegram', reply_markup = back_to_start_btn)
...
☎️ Numero: {numero}
__ℹ️ Hai superato il limite di tempo [150 secondi], il VoIP è stato revocato e il saldo è stato riaggiunto__''') 
        return False 

async def get_code(query, message):
    global idnumero
    global idutente
    print(f'codice richiesto da {idutente}')
    global numero
...
async def buynum(client, message): 
    ...
    id = response.split(':')[1]
    pass
    print(f'Nuovo acquisto da {message.chat.id} | {id} | {number}')
    idutente = f'{message.chat.id}'
    idnumero = f'{id}'
    numero = f'{number}'
    await app.send_message(chat_id = message.chat.id, text =  f**✅ Numero acquistato** | ({id}) ☎️ Numero: +{number} !) 
...
async def get_code(client, message):
    global idnumero
    print(f'codice richiesto da {message.chat.id}')
    global idutente
    global numero
    response = requests.get('https://api.sms-activate.org/stubs/handler_api.php?api_key=' + phoneAPI + '&action=getStatus&id=' + idnumero).text
    if 'STATUS_OK' not in response:
